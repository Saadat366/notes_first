...
@app.route("/table")
def table():
    notes_file = open("notes.txt", "r", encoding="utf8")
    rows = [[row[:10], row[10:].strip()] for row in notes_file] #strip метод, который удаляет переносы сторки пробелы справа и слева
    notes_file.close()
